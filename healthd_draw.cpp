...
void HealthdDraw::draw_date(const animation* anim) {
    ...
    static constexpr int DATE_LENGTH = 11;
    ...
    size_t length = strftime(date_str, DATE_LENGTH, DATE_FORMAT, time_info);
    if (length < 1) {
        LOGE("Could not format date\n");
        return;
    }

    int x, y;
    // Reaproveitamos o método de cálculo de posição, mas deslocamos a data para abaixo do relógio
    determine_xy(field, length, &x, &y);
    y += field.font->char_height + 10; // Adiciona espaçamento abaixo do relógio

    LOGV("drawing date %s %d %d\n", date_str, x, y);
    gr_color(field.color_r, field.color_g, field.color_b, field.color_a);
    draw_text(field.font, x, y, date_str);
}
